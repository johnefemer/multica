...
Private GitHub access for local `git` / `multica repo checkout` uses tokens **on the daemon host**, not the workspace GitHub OAuth integration. See **[GitHub authentication for agents](/github-auth-for-agents)** for the three-tier token priority, how to verify `gh` on a runtime, workspace repos, and per-identity workarounds.
