---
title: 故障排查
description: self-host Agenthost 遇到的 Top 7 常见问题——症状、原因、怎么查、怎么修。
---
...
**症状**：[`agenthost daemon`](/cli) 的 `status` 命令显示 `offline` 或 `connection refused`；服务器日志里没有 `/api/daemon/register` 或 `/api/daemon/heartbeat` 的请求。守护进程机制详见 [守护进程与运行时](/daemon-runtimes)。
...
3. **Token 过期或无效** —— 你从来没跑过 `agenthost login`，或 PAT 被撤销
...
```bash
agenthost daemon logs --lines 100    # 看 daemon 侧错误
...
agenthost workspace list            # 确认你是目标工作区成员
```

**怎么修**：按上面原因对症处理。最常见的两个是**改 `MULTICA_SERVER_URL` 重启 daemon**（`agenthost daemon restart`）和**重新登录**（`agenthost logout && agenthost login`）。

## 任务一直卡在 queued

**症状**：把 issue 分给 agent 后，issue 状态立刻变 `in_progress`，但过了很久页面没有 agent 执行的迹象；`agenthost daemon status` 显示 daemon `online`。
...
```bash
agenthost daemon status --output json       # runtime 列表 + last_seen_at
agenthost agent list                         # 查 agent 的 archived 状态
agenthost issue show <issue-id>             # 看 task 历史
```
...
- 并发打满 → 等现有任务跑完，或 `agenthost agent update <id> --max-concurrent-tasks 10` 提升上限
- 同 issue 串行 → 等前一个任务结束，或改分给不同 agent
- Agent 被 archive → `agenthost agent restore <id>`
- Runtime 未注册 → `agenthost daemon restart`，daemon 会重新注册
...
- Origin 错 → 在 server 的 `.env` 设 `FRONTEND_ORIGIN=https://agenthost.yourdomain.com`（或逗号分隔的 `CORS_ALLOWED_ORIGINS`），重启 server
...
**症状**：`agenthost server` 或 `agenthost daemon start` 启动失败，报 `address already in use`。
...
| 组件 | 位置 | 命令 |
|---|---|---|
| **守护进程** | `~/.multica/daemon.log`（后台模式）或前台 stdout | `agenthost daemon logs -f --lines 100` |
| **服务器（Docker）** | container stdout | `docker logs -f <container>` |
| **服务器（systemd）** | journal | `journalctl -u agenthost-server -f` |
| **前端（dev）** | `pnpm dev` 所在终端 | 直接看 |
| **前端（browser）** | DevTools → Console | 按 `F12` |

需要更详细的 daemon 日志，把它从后台挪到前台跑：`agenthost daemon stop && agenthost daemon start --foreground`。
