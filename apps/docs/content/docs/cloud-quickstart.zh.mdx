...
这一页带你走一遍 Agenthost Cloud 的端到端流程——**注册 → 装 [命令行工具](/cli) → 启动 [守护进程](/daemon-runtimes) → 创建 [智能体](/agents) → 分配第一个 [任务](/tasks)**，约 5 分钟完成。
...
```bash
agenthost issue create --title "给 README 加一段 ASCII 架构图"
```
...
4. 结束后把结果发回 Agenthost（状态变 `completed` 或 `failed`，根据是否自动重试）
