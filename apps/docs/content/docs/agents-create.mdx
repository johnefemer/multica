...
Each runtime is backed by a specific AI coding tool. Agenthost supports 10 of them. The most common choices:
...
**Values in `custom_env` are stored in plaintext in Agenthost's server database.** Non-creators and non-workspace-admins can't see the values (the API returns `****`), but they're still visible in database backups and DB audits.
