...
description: Run Agenthost on your own server or machine with Docker. Takes about 10 minutes.
---

import { Callout } from "fumadocs-ui/components/callout";

This page walks you through running the Agenthost **server** (backend + frontend + PostgreSQL) on your own machine or server with Docker. When you're done, your data is fully under your control — including [workspaces](/workspaces), [issues](/issues), [comments](/comments), and [agent](/agents) configuration.
...
2. Pull the official Docker images (PostgreSQL, Agenthost backend, Agenthost frontend)
...
The CLI install is the same as in [Cloud quickstart → 2. Install the CLI](/cloud-quickstart#2-install-the-agenthost-cli) — Homebrew / script / PowerShell, pick one. Once installed, **use the self-host variant of the setup command**:

```bash
agenthost setup self-host --server-url http://<your-server-address>:8080 --app-url http://<your-server-address>:3000
```
