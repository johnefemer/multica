...
Agenthost supports two sign-in methods: **email + verification code** (default) and **Google OAuth** (optional). On successful sign-in, the server issues a JWT cookie with a 30-day lifetime. This page covers how to configure each method, how to restrict who can sign up, and the single biggest trap for self-hosted deployments.
...
Agenthost has a development-only master code, `888888` — a backdoor so local development doesn't depend on Resend. The rule is straightforward: when `APP_ENV != "production"`, **any email** plus `888888` passes verification.
...
2. Set the **Authorized redirect URIs** to your Agenthost frontend address plus `/auth/callback`, for example:

    ```text
    https://agenthost.yourdomain.com/auth/callback
    ```
...
3. Once you have the client ID and client secret, set three environment variables:
    ...
    GOOGLE_REDIRECT_URI=https://agenthost.yourdomain.com/auth/callback
    ```
...
**Only people who already have a Agenthost account.** Accepting an invite doesn't check the signup allowlist — if the invitee has signed up already (for example in another workspace), clicking the invite link and signing in lets them accept.
