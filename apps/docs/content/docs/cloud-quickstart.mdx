...
This page walks you end-to-end through Agenthost Cloud — **sign up → install the [CLI](/cli) → start the [daemon](/daemon-runtimes) → create an [agent](/agents) → assign your first [task](/tasks)**. Takes about 5 minutes.
...
`agenthost setup` will:

1. Configure the CLI to connect to Agenthost Cloud
...
<Callout type="info">
**Using the desktop app?** The desktop app **starts the daemon automatically** on launch — no need to run `agenthost setup` by hand. See [Desktop app](/desktop-app).
</Callout>
...
```bash
agenthost issue create --title "Add an ASCII architecture diagram to the README"
```

Assign the issue to the agent you just created — click its avatar in the web UI, or use the CLI:

```bash
agenthost issue assign MUL-1 --to my-agent-name
```
...
4. When done, it reports the result back to Agenthost (status becomes `completed` or `failed`, depending on whether auto-retry kicks in)
