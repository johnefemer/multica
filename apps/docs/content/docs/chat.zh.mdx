...
- 在自己的工作目录里读写文件
- 调用不需要 issue 上下文的 `agenthost` CLI 命令（比如查询工作区基本信息）

**不能做的**：

- **看到任何 issue**——智能体收到的提示里没有 issue ID，`agenthost issue list` 之类命令对它返回空
...
如果**某一轮失败**，Agenthost 会查找上一轮建立过 session ID 的任务（不论它当时成功还是失败）并尝试 resume——不会因为中间一次出错就丢掉整段对话的记忆。
