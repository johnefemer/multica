...
description: The top 7 common issues when self-hosting Agenthost — symptoms, causes, how to diagnose, how to fix.
---
...
**Symptom**: [`agenthost daemon`](/cli)'s `status` command shows `offline` or `connection refused`; the server logs show no `/api/daemon/register` or `/api/daemon/heartbeat` requests. For how the daemon mechanism works, see [Daemon and runtimes](/daemon-runtimes).
...
3. **Token expired or invalid** — you never ran `agenthost login`, or the PAT was revoked
...
```bash
agenthost daemon logs --lines 100    # look for daemon-side errors
...
agenthost workspace list            # confirm you're a member of the target workspace
```

**How to fix**: address each cause above. The two most common fixes are **changing `MULTICA_SERVER_URL` and restarting the daemon** (`agenthost daemon restart`) and **signing in again** (`agenthost logout && agenthost login`).

## Tasks stuck in `queued`

**Symptom**: after assigning an issue to an agent, the issue status flips to `in_progress` immediately, but a long time passes with no sign of agent execution on the page; `agenthost daemon status` shows the daemon `online`.
...
```bash
agenthost daemon status --output json       # runtime list + last_seen_at
agenthost agent list                         # check agent archived state
agenthost issue show <issue-id>             # inspect task history
```
...
- Concurrency full → wait for running tasks to finish, or `agenthost agent update <id> --max-concurrent-tasks 10` to raise the ceiling
- Same-issue serialization → wait for the previous task to finish, or reassign to a different agent
- Agent archived → `agenthost agent restore <id>`
- Runtime not registered → `agenthost daemon restart`, and the daemon will re-register
...
- Wrong origin → set `FRONTEND_ORIGIN=https://agenthost.yourdomain.com` in the server's `.env` (or comma-separated `CORS_ALLOWED_ORIGINS`) and restart the server
...
**Symptom**: `agenthost server` or `agenthost daemon start` fails with `address already in use`.
...
|---|---|---|
| **Daemon** | `~/.multica/daemon.log` (background mode) or foreground stdout | `agenthost daemon logs -f --lines 100` |
| **Server (Docker)** | Container stdout | `docker logs -f <container>` |
| **Server (systemd)** | journal | `journalctl -u agenthost-server -f` |
| **Frontend (dev)** | Terminal running `pnpm dev` | Read directly |
| **Frontend (browser)** | DevTools → Console | Press `F12` |

For more detailed daemon logs, move it from background to foreground: `agenthost daemon stop && agenthost daemon start --foreground`.
