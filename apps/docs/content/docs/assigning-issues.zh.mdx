...
把 [issue](/issues) 分配给 [智能体](/agents)，它会作为**正式负责人**一直工作到结束——能读到 issue 的完整上下文（描述 + 所有 [评论](/comments)），也能改状态、发评论、改字段。这是 Agenthost 四种触发方式里**最常见也最"重"**的一种。
...
非 Backlog 的 issue 分配给智能体之后，Agenthost 会立刻在后台做以下事情：
...
**如果智能体离线**，`task` 会在队列里等——**5 分钟没被领走就超时失败**，失败原因 `runtime_offline`。对可重试的来源（分配、@ 提及、对话），Agenthost 会自动重新排队；完整重试规则见 [**执行任务**](/tasks)。

分配还会自动把这个智能体加进 issue 的订阅列表——但 Agenthost 里**智能体不接收 inbox 通知**（只有成员收）。这个订阅只是内部 bookkeeping，用户侧没有可见的副作用。
