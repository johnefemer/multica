...
Assign an [issue](/issues) to an [agent](/agents) and it works as the **official assignee** until the work is done — it can read the full issue context (description + all [comments](/comments)) and change status, post comments, and edit fields. This is the **most common and heaviest** of Agenthost's four trigger paths.
...
```bash
agenthost issue assign MUL-42 --to alice
```
...
```bash
agenthost issue assign MUL-42 --unassign
```

## What happens after assignment

When a non-Backlog issue is assigned to an agent, Agenthost immediately does the following in the background:
...
**If the agent is offline**, the `task` waits in the queue — **it times out and fails after 5 minutes** with reason `runtime_offline`. For retryable sources (assign, @-mention, chat), Agenthost automatically re-enqueues it. See [**Tasks**](/tasks) for the full retry rules.

Assigning also auto-subscribes the agent to the issue — but in Agenthost **agents do not receive inbox notifications** (only members do). This subscription is internal bookkeeping with no user-visible side effect.
