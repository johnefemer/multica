...
description: Deploy Agenthost on your own infrastructure.
---

## Architecture Overview

Agenthost has three components:
...
Each user who wants to run AI agents locally also installs the **`agenthost` CLI** and runs the **agent daemon** on their own machine.
...
This installs the CLI, checks out the latest self-host assets, pulls the official Agenthost images from GHCR, and configures everything for localhost. Then open http://localhost:3000 and pick a login method: configure `RESEND_API_KEY` in `.env` for email-based codes (recommended), or set `APP_ENV=development` in `.env` to enable the dev master code **`888888`**. See [Step 2 — Log In](#step-2--log-in) for details.
...
<Callout>
Alternatively, configure step by step: `agenthost config set server_url http://localhost:8080 && agenthost config set app_url http://localhost:3000 && agenthost login && agenthost daemon start`
</Callout>
...
## Switching to Agenthost Cloud

If you've been self-hosting and want to switch your CLI to [Agenthost Cloud](https://multica.ai):
...
Agenthost uses email-based magic link authentication via [Resend](https://resend.com).
...
Agenthost requires PostgreSQL 17 with the pgvector extension.
...
This starts a `pgvector/pgvector:pg17` container on port 5432 with default credentials (`agenthost`/`agenthost`).
