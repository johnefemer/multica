...
Agents run on **your** machine via the [daemon](/daemon-runtimes). Cloning and fetching private GitHub repositories happens there, using **git** and optional **`gh`**. The Agenthost server does not perform git operations for agent tasks.
