...
// `no such host` errors when calling out (for example, `agenthost issue get`
...
const (
	agenthostManagedBeginMarker = "# BEGIN agenthost-managed (do not edit; regenerated by daemon)"
	agenthostManagedEndMarker   = "# END agenthost-managed"
	// Legacy markers from before the agenthost rename — still matched so old
	// config.toml files are migrated cleanly on the next daemon run.
	legacyManagedBeginMarker = "# BEGIN multica-managed"
	legacyManagedEndMarker   = "# END multica-managed"
)

// renderAgenthostManagedBlock produces the managed block for the given policy.
func renderAgenthostManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(agenthostManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(agenthostManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}

// managedBlockRe captures both the current agenthost-managed block and the
// legacy multica-managed block so old config.toml files are migrated cleanly.
var managedBlockRe = regexp.MustCompile(
	`(?ms)^(?:` +
		regexp.QuoteMeta(agenthostManagedBeginMarker) + `|` +
...
// written managed block (including old multica-managed blocks) is removed
// first; user content outside the markers is preserved.
func upsertAgenthostManagedBlock(content string, policy codexSandboxPolicy) string {
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderAgenthostManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the agenthost-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertAgenthostManagedBlock(existing, policy)
